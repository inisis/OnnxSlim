...
def pytest_sessionfinish(session, exitstatus):
    if any("test_benchmark.py" in str(item.nodeid) for item in session.items):
        ...
        for name, summary in TestModelZoo.results.items():
            ...
            op_number[name] = {k: sum(count for op, count in v.op_type_counts.items() if op != "Constant") for k, v in summary.items() if v is not None}
        ...
        for ax_idx, src_model_key_iter in enumerate(ANALYSIS_METRICS):
            ...
            for tool_idx, lang_cat_key in enumerate(optimization_tools):
                ...
                bars = ax.bar(x_category_centers + offset, y_values, bar_width,
                            label=lang_cat_key,
                            color=model_color_map[lang_cat_key],
                            hatch='////' if lang_cat_key == 'float' else None, edgecolor='black')

                for bar in bars:
                    yval = bar.get_height()
                    if yval > 0.01:
                        ax.text(bar.get_x() + bar.get_width()/2, yval + 0.01 * ax.get_ylim()[1], f'{yval:.2f}',
            ...
            ax.set_title(f'{src_model_key_iter.replace("_", " ").title()} Compression Ratio by Optimization Tool', fontsize=14)
