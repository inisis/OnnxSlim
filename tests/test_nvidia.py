...
MODELZOO_PATH = "/data/modelzoo/nvidia"
...
class TestModelZoo:
    def test_issue1_repro_io_tensors_shape_dtype(self, request):
    ...
    def test_issue1_repro_mode_error_mobilenetv1(self, request):
        name = request.node.originalname[len("test_") :]
        filename = f"{MODELZOO_PATH}/{name}.onnx"
    ...
    def test_issue2_repro_gemm_invalid_shape(self, request):
        name = request.node.originalname[len("test_") :]
        filename = f"{MODELZOO_PATH}/{name}.onnx"
    ...
    def test_issue2_repro_mul_incompatible_dimensions(self, request):
        name = request.node.originalname[len("test_") :]
        filename = f"{MODELZOO_PATH}/{name}.onnx"
    ...
    def test_issue3_repro_conv_bn_fusion(self, request):
        name = request.node.originalname[len("test_") :]
        filename = f"{MODELZOO_PATH}/{name}.onnx"
    ...
    def test_issue3_repro_conv_resize_issue(self, request):
        name = request.node.originalname[len("test_") :]
        filename = f"{MODELZOO_PATH}/{name}.onnx"
