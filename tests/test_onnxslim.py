...
class TestFeat:
    def test_input_shape_modification(self, request):
        """Test the modification of input shapes for a UNet model using the onnxslim command."""
    ...
    def test_fp162fp32_conversion(self, request):
        """Test the conversion of an ONNX model from FP16 to FP32 using the onnxslim tool with specified input shapes."""
        filename = download_onnx_from_url(f"http://120.224.26.32:15030/aifarm/onnx/UNetModel-fp16.onnx")
