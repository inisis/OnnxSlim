...
try:
    ...
    try:
        ...
        class tqdm(object):  # type: ignore[no-redef]
            ...
            def update(self, n):
                """Updates the progress by incrementing the counter 'n' by a specified amount."""
                if self.disable:
                    return
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                """Exits the runtime context related to this object, cleaning up if the progress indicator is enabled."""
                if self.disable:
                    return
...
def _load_attr_from_module(module, func_name):
    """Load an attribute by name from a module if it exists."""
...
def _get_torch_home():
    """Get the directory path where Torch caches data."""
...
def _parse_repo_info(github):
    """Parse the GitHub repository information and determine the default branch if not specified."""
    if ":" in github:
...
def _download_url_to_file(url, dst, hash_prefix=None, progress=True):
    """Download a file from a URL to a destination path with an optional hash prefix and progress display."""
    warnings.warn(
...
def _is_legacy_zip_format(filename):
    """Checks if the given zip file is in the legacy format, expecting only one non-directory file."""
...
def _legacy_zip_load(filename, model_dir, map_location):
    """Load a legacy zip file, extract its contents, and load the extracted file using torch.load()."""
    warnings.warn(
